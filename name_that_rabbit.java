...
public class Answer {
    public static String[] answer(String[] names) {
        Arrays.sort(names, new StringSumComparator());
        return names;
    }
    static class StringSumComparator implements Comparator<String> {
        public int compare(String s1, String s2) {
            int a = getStringSum(s1);
            int b = getStringSum(s2);
            int c = b - a;
            if (c == 0) {
                c = s2.compareTo(s1);    
            }
        ...
        private int getStringSum(String s1) {
